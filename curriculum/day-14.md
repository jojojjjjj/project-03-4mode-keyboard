...
**电源走线与铺地（双层板）Power Routing & Ground Pour (2-layer):**
...
Bottom (L2): 大面积地铜铺铜
  ...
  │  底层空白处，加粗到 0.5mm 以上    │
  └──────────────────────────────┘
  双层板没有专门的电源平面，用粗走线 + 铺地铜代替。
  避免 signal 走线跨越地铜的缺口（否则回流路径变长，EMI 增大）。
```
